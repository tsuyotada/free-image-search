...
export default function HomeClient({ initialHeroImage }: { initialHeroImage: string | null }) {
  ...
  const [heroImage] = useState<string | null>(initialHeroImage)
  ...
  // Track hero image load — handles both cache-hit (img.complete) and network cases
  useEffect(() => {
    if (!heroImage) return
    const img = new window.Image()
    img.onload = () => setHeroImageLoaded(true)
    img.src = heroImage
    if (img.complete) setHeroImageLoaded(true)
  }, [heroImage])
      ...
      {heroPhase !== "hidden" && (
        <div
          style={{
            position: "fixed",
            inset: 0,
            // Fallback gradient — visible instantly before image loads
        ...
        >
          {/* Photo layer: fades in once image is downloaded */}
          {heroImage && (
            <div
              style={{
                position: "absolute",
